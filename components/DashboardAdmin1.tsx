  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4 border-b border-violet-100">
        <button 
          onClick={() => { setActiveTab('all'); setViewingReportsOf(null); }}
        ...
        <button 
          onClick={() => { setActiveTab('reclassification'); setViewingReportsOf(null); }}
        ...
        <button 
          onClick={() => { setActiveTab('closures'); setViewingReportsOf(null); }}
        ...
        <button 
          onClick={() => { setActiveTab('notes'); setViewingReportsOf(null); }}
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          {viewingReportsOf ? (
            <div className="bg-white rounded-[2rem] shadow-sm border border-violet-100 p-8 animate-in fade-in zoom-in-95">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-black text-violet-900 uppercase">Informes del Caso #{viewingReportsOf.id}</h3>
                <button onClick={() => setViewingReportsOf(null)} className="text-[10px] font-black uppercase text-gray-400 tracking-widest hover:text-gray-600">Volver</button>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-violet-50 text-violet-800 text-[10px] font-black uppercase tracking-widest">
                    <tr>
                  ...
                  <tbody className="divide-y divide-violet-50">
                    <tr>
                      <td className="px-6 py-4 font-bold text-sm">Informe 1</td>
                      <td className="px-6 py-4 text-xs text-gray-500">
                        {viewingReportsOf.informe1Url ? "Documento cargado por el despacho" : "No disponible"}
                      </td>
                      <td className="px-6 py-4">
                        {viewingReportsOf.informe1Url ? (
                          <a href={viewingReportsOf.informe1Url} target="_blank" rel="noopener noreferrer" className="bg-violet-100 text-violet-700 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase flex items-center gap-2 w-fit">
                            <ICONS.PdfIcon /> Descargar PDF
                          </a>
                        ) : "-"}
                      </td>
                    </tr>
                    <tr>
                      <td className="px-6 py-4 font-bold text-sm">Informe 2</td>
                      <td className="px-6 py-4 text-xs text-gray-500">
                        {viewingReportsOf.informe2Url ? "Documento cargado por el despacho" : "No disponible"}
                      </td>
                      <td className="px-6 py-4">
                        {viewingReportsOf.informe2Url ? (
                          <a href={viewingReportsOf.informe2Url} target="_blank" rel="noopener noreferrer" className="bg-violet-100 text-violet-700 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase flex items-center gap-2 w-fit">
                            <ICONS.PdfIcon /> Descargar PDF
                          </a>
                        ) : "-"}
                      </td>
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-violet-100 overflow-hidden">
              <table className="w-full text-left">
                      ...
                      {cases.map(c => (
                        <tr key={c.id} className="hover:bg-violet-50/20 transition-colors">
                          ...
                          <td className="px-6 py-4 flex items-center gap-2">
                            ...
                            <button onClick={() => setViewingReportsOf(c)} className="bg-white border-2 border-violet-100 text-violet-600 px-4 py-2 rounded-xl text-[10px] uppercase font-black hover:bg-violet-50 transition-all">Ver Informes</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                {/* OTRAS PESTAÑAS SIMILARES... (manteniendo coherencia) */}
                      ...
                      {notes.map(n => (
                        <tr key={n.id} className="hover:bg-blue-50/20 transition-colors">
                          ...
                          <td className="px-6 py-6 text-right align-top">
                            <button onClick={() => handleDeleteNote(n.id)} className="text-red-400 hover:text-red-600 transition-all p-2 bg-red-50 rounded-xl hover:shadow-md">
            ...
            <div className="bg-white rounded-[2rem] shadow-2xl border border-violet-100 p-8 space-y-6 animate-in slide-in-from-right-8 sticky top-24">
              ...
              {selectedCase.estado === CaseStatus.RECLASIFICACION_SOLICITADA ? (
                <div className="space-y-5">
                  {/* ... (anterior lógica de reclasificación) ... */}
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea className="w-full p-4 bg-violet-50 rounded-2xl border-none text-sm font-medium text-violet-900 focus:ring-2 focus:ring-violet-500 placeholder:text-violet-300" rows={5} value={adminAnalysis} onChange={e => setAdminAnalysis(e.target.value)} />
                  </div>
                  <button disabled={loading || !adminAnalysis} onClick={handleEscalateToAdmin2} className="w-full bg-violet-700 text-white font-black py-5 rounded-2xl shadow-xl uppercase text-xs tracking-widest hover:bg-violet-800 transition-all active:scale-95">
                    {loading ? 'GENERANDO AUTO...' : 'ESCALAR A SECRETARÍA DE GOBIERNO'}
                  </button>
                </div>
              ) : selectedCase.estado === CaseStatus.CIERRE_SOLICITADO ? (
                <div className="space-y-6">
                  {/* ... (anterior lógica de cierre) ... */}
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <button disabled={loading} onClick={handleAcceptClosure} className="w-full bg-green-600 text-white font-black py-4 rounded-2xl shadow-lg uppercase text-xs tracking-widest hover:bg-green-700 transition-all">ACEPTAR CIERRE DEFINITIVO</button>
                    <div className="space-y-2 pt-4 border-t border-gray-100">
                      ...
                      <textarea className="w-full p-4 bg-gray-50 rounded-2xl border-none text-sm font-medium text-gray-900 focus:ring-2 focus:ring-violet-500 placeholder:text-gray-300" rows={3} value={closureJustification} onChange={e => setClosureJustification(e.target.value)} />
                    </div>
                    <button disabled={loading || !closureJustification} onClick={handleDenyClosure} className="w-full bg-red-600 text-white font-black py-4 rounded-2xl shadow-lg uppercase text-xs tracking-widest hover:bg-red-700 transition-all">NEGAR CIERRE (EMITIR AUTO)</button>
                  </div>
                </div>
              ) : (
                <div className="space-y-6">
                  {/* ... (lógica normal de asignación) ... */}
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-1">Asignar a Despacho</label>
                    <select className="w-full p-4 bg-violet-50 rounded-2xl border-none text-sm font-bold text-violet-900 focus:ring-2 focus:ring-violet-500 appearance-none" value={assignment.despacho} onChange={e => setAssignment({...assignment, despacho: e.target.value})}>
                  ...
                  <div className="space-y-2">
                    ...
                    <select className="w-full p-4 bg-violet-50 rounded-2xl border-none text-sm font-bold text-violet-900 focus:ring-2 focus:ring-violet-500 appearance-none" value={assignment.urgencia} onChange={e => setAssignment({...assignment, urgencia: e.target.value as UrgencyLevel})}>
                  ...
                  <div className="space-y-2">
                    ...
                    <select className="w-full p-4 bg-violet-50 rounded-2xl border-none text-sm font-bold text-violet-900 focus:ring-2 focus:ring-violet-500 appearance-none" value={assignment.estado} onChange={e => setAssignment({...assignment, estado: e.target.value as CaseStatus})}>
                  ...
                  <button onClick={handleAssign} className="w-full bg-violet-700 text-white font-black py-5 rounded-2xl uppercase text-xs tracking-widest shadow-xl hover:bg-violet-800 transition-all active:scale-95 mt-4">GUARDAR CAMBIOS Y NOTIFICAR</button>
                </div>
              )}
            </div>
          ) : !viewingReportsOf && (
